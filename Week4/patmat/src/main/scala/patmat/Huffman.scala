...
trait Huffman extends HuffmanInterface {
    ...
    if (chars.isEmpty) Nil
      ...
      pair match {
        case (theChar, theInt) =>
          ...
          lazy val size = chars.size - tail.size
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    def insert(leaf: Leaf, list: List[Leaf]): List[Leaf] = list match {
      case List() => List(leaf)
      case y :: ys =>
        if (leaf.weight <= y.weight) leaf :: list else y :: insert(leaf, ys)
    }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    // If trees less than 2 then return it self as there is nothing to combine
    if (trees.size < 2) trees
    else { //Combine the first two heads (tree.head and tree.tail.head)
      val head = trees.head
      val tailOfHead = trees.tail.head
      val fork = makeCodeTree(head, tailOfHead)

      val tail = trees.tail.tail // Remove the fist 2 elements of the list
      def insert(codeTree: CodeTree, tail: List[CodeTree]): List[CodeTree] =
        tail match { //If the List has only 2 elements tail will be empty "List()"
          case List() => fork :: tail
          case head :: ys =>
            if (fork.weight <= weight(head)) fork :: tail
            else head :: insert(fork, ys)
        }
      insert(fork, tail)
    }
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def fork(tree: CodeTree, bits: List[Bit]): (Char, List[Bit]) =
      tree match {
        case Fork(left, right, _, _) =>
          bits match {
            case 0 :: tail => fork(left, tail)
            case 1 :: tail => fork(right, tail)
            case _ =>
              throw new IllegalArgumentException("Incomplete bit sequence")
          }
        case Leaf(char, _) => (char, bits)
      }
    fork(tree, bits) match {
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def fork(tree: CodeTree, char: Char): List[Bit] = tree match {
      case Leaf(_, _) => Nil
      case Fork(left, right, _, _) =>
        if (chars(left) contains char) 0 :: fork(left, char)
        else 1 :: fork(right, char)
    }
    text.flatMap(fork(tree, _))
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
    ...
    case (x, bits) :: tail => if (x == char) bits else codeBits(tail)(char)
  }
